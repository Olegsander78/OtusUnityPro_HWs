...
namespace PolyAndCode.UI
{
    ...
    /// </summary>
    public class VerticalScroller<T> : IScroller where T : Component
    {
        ...
        public VerticalScroller(RectTransform viewport, RectTransform content,
            IDataAdapter<T> dataAdapter, bool isGrid, int coloumns)
        {
        ...
        private void CreateCellPool()
        {
            ...
            if (IsGrid)
            {
                SetTopLeftAnchor(prototypeTransform);
            }
            else
            {
                SetTopAnchor(prototypeTransform);
            }
            ...
            _cellHeight = prototypeTransform.sizeDelta.y / prototypeTransform.sizeDelta.x * _cellWidth;
            ...
            while ((poolSize < minPoolSize || currentPoolCoverage < requriedCoverage) &&
                   poolSize < DataAdapter.DataCount)
            {
                ...
                DataAdapter.OnAttachItem(_cachedCells[_cachedCells.Count - 1], poolSize);
        ...
        private Vector2 RecycleTopToBottom()
        {
            ...
            while (_cellPool[topMostCellIndex].MinY() > _recyclableViewBounds.max.y &&
                   currentItemCount < DataAdapter.DataCount)
            {
                if (IsGrid)
                {
                    if (++_bottomMostCellColoumn >= _coloumns)
                ...
                //Cell for row at
                DataAdapter.OnAttachItem(_cachedCells[topMostCellIndex], currentItemCount);
        ...
        private Vector2 RecycleBottomToTop()
        {
                   ...
                   currentItemCount > _cellPool.Count)
            {
                ...
                //Cell for row at
                DataAdapter.OnAttachItem(_cachedCells[bottomMostCellIndex], currentItemCount - _cellPool.Count);
